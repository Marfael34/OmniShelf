...
const AddToCollectionModal = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={closeCollectionModal} />
      <div className="relative bg-bg-surface border border-white/10 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
          <h2 className="text-xl font-black tracking-tighter uppercase">Ajouter à une collection</h2>
          <button onClick={closeCollectionModal} className="text-text-dim hover:text-white"><X size={24} /></button>
        </div>
        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
          ...
          {isCreating ? <NewCollectionForm name={newName} setName={setNewName} onCreate={() => createMutation.mutate(newName)} onCancel={() => setIsCreating(false)} isPending={createMutation.isPending} /> : 
