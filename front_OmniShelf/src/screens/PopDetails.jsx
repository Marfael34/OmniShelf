...
const PopDetails = () => {
  ...
  const { data: pop, isLoading, error } = useQuery({
    queryKey: ["pop", id],
    queryFn: async () => {
      const res = await api.get(`/proxy/details?external_id=${id}&category=pop`);
      return res.data.data;
    },
    enabled: !!id,
  });

  const { amazon, fnac } = useAffiliationLink(`Funko POP ${pop?.product_name || pop?.character || ""}`, "pop", id);
    ...
    <div className="max-w-6xl mx-auto py-12 px-4 space-y-8">
      ...
      <div className="bg-(--bg-bg-surface) rounded-3xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col md:flex-row relative">
        <div className="md:w-1/3 bg-white flex items-center justify-center p-12">
          <img src={pop.image_url || pop.image} alt={pop.product_name} className="w-full h-full object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] transform hover:scale-105 transition-transform duration-500" />
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center space-x-4">
                <span className="bg-teal-900/20 text-teal-400 px-4 py-2 rounded-xl text-sm font-black border border-teal-500/20 uppercase tracking-widest">
                    {pop.categories_tags?.[0]?.replace('en:', '').replace(/-/g, ' ') || "Figurine Collection"}
                </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 pt-6 border-t border-gray-800/50">
              <div className="space-y-1">
                ...
                <div className="text-sm font-bold text-text-main">{pop.code || pop._id}</div>
              </div>
              <div className="space-y-1">
                <div className="flex items-center text-text-dim text-xs font-bold uppercase tracking-widest mb-1">
                    <Tag size={12} className="mr-2" /> Marque
                </div>
                <div className="text-sm font-bold text-text-main">{pop.brands || "Funko"}</div>
              </div>
            </div>
          </div>
