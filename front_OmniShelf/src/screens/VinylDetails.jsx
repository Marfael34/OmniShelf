...
const VinylDetails = () => {
  ...
  const { data: release, isLoading, error } = useQuery({
    queryKey: ["vinyl", id],
    queryFn: async () => {
      return proxyService.getDetails(id, "vinyl");
    },
    enabled: !!id,
  });

  const { amazon, fnac } = useAffiliationLink(`${release?.artist} ${release?.title}`, "vinyl", id);
      ...
      <div className="bg-bg-surface rounded-3xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col md:flex-row relative">
        <div className="md:w-1/2 bg-gray-900 min-h-[400px] flex items-center justify-center p-12">
          <div className="relative group w-full max-w-[320px]">
            <div className="absolute -inset-4 bg-purple-500 blur-3xl opacity-10 group-hover:opacity-40 transition-opacity"></div>
        ...
        <div className="p-10 md:w-1/2 flex flex-col justify-between space-y-8">
          <div className="space-y-6">
            <div>
              ...
              <h2 className="text-2xl text-purple-400 font-bold">{release.artist}</h2>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
                ...
                {release.releaseYear && (
                    <span className="bg-gray-800 text-gray-300 px-3 py-1 rounded-lg text-xs font-black border border-gray-700 uppercase tracking-wider">{release.releaseYear}</span>
                )}
            </div>
