...
const dummyWishlist = [
  { id: "w1", title: "Final Fantasy XVI", category: "game", added: "2023-10-01" },
  { id: "w2", title: "Berserk Tome 1", category: "manga", added: "2023-10-05" },
  { id: "w3", title: "Daft Punk - Discovery", category: "vinyl", added: "2023-10-10" }
];
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-extrabold text-(--text-main)">Wishlist Commune</h1>
          <p className="text-(--text-dim) mt-2">Retrouvez tous vos souhaits au même endroit.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {filteredItems.map(item => (
          <div 
            key={item.id} 
            className="bg-(--bg-surface) rounded-xl overflow-hidden border border-gray-800 flex items-center p-4 space-x-4"
          >
            ...
            <Link 
              ...
              className="p-2 bg-(--color-primary) text-(--color-accent) rounded-full hover:bg-(--color-accent) hover:text-(--bg-main) transition-colors"
            >
              Voir
            </Link>
          </div>
