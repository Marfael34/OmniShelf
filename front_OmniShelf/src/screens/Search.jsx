  ...
  return (
    <div className="max-w-6xl mx-auto py-12 space-y-12 px-4">
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <p className="text-xl text-text-dim font-medium">Explorez des millions de références (RAWG, Discogs, Google Books).</p>
        
        <form 
          onSubmit={handleSearch}
          className="flex bg-bg-surface rounded-2xl border-2 border-gray-800 overflow-hidden shadow-2xl mt-8 focus-within:border-accent transition-all p-1"
        >
          <select 
            className="bg-gray-800 text-text-main px-6 py-4 outline-none border-r border-gray-700 font-bold rounded-xl"
            value={category}
          ...
          <input 
            ...
            className="flex-1 bg-transparent text-text-main px-6 py-4 outline-none font-medium"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <button 
            type="submit"
            className="bg-accent text-bg-bg-main px-10 py-4 font-black rounded-xl hover:opacity-90 transition-all flex items-center space-x-2"
          >
      ...
      <div className="pt-8">
        <div className="flex justify-between items-center mb-10">
            ...
            {results.length > 0 && <span className="text-text-dim font-bold">{results.length} références trouvées</span>}
        </div>
        ...
        {isLoading ? (
            ...
            <div className="text-center py-24 bg-bg-surface rounded-3xl border-2 border-dashed border-gray-800">
                ...
                <p className="text-xl text-text-dim font-bold">Entrez un titre pour lancer la recherche multi-sources.</p>
            </div>
        )}
      </div>
    </div>
