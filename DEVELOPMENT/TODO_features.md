...
- [x] **Stratégie Offline (Workbox)** :
  - [x] Configurer `vite-plugin-pwa` pour le mode offline partiel.
  - [x] Mettre en cache la collection de l'utilisateur dans IndexedDB pour un affichage immédiat sans réseau.
- [x] **Hydratation & Performance** :
  - [x] Régler le `stale-time` de React Query pour dédupliquer les appels et limiter la charge sur le serveur Symfony BFF.

## 5. QA & Plan de Test

- [x] Tester le "Cold Start" : s'assurer que des suggestions populaires s'affichent si la collection est vide.
- [x] Tester la concomitance : scanner rapidement deux fois le même code-barres ne doit pas créer de doublon (tester la contrainte d'unicité côté backend).
- [x] Vérifier la stricte interdiction de TypeScript et le respect de React Compiler dans les nouveaux fichiers générés.
